...
from devops.core.utils.MailTask import MailTask
...
class LogParser:
    ...
    def getDynamicVars2(self, petit_group):
        ...
        petit_group['event_label'] = petit_group['event_label'].map(lambda x: ' '.join(
    ...
    def outputResult(self):
        self.df_log.to_csv(os.path.join(
            self.savePath, self.logName + '_structured.csv'), index=False)

    def load_data(self):
        headers, regex = self.generate_logformat_regex(self.log_format)

        self.df_log = self.log_to_dataframe(os.path.join(
            self.path, self.logname), regex, headers, self.log_format)
    ...
    def parse(self, logname):
        ...
        for i in keys:
            ...
            # print(slc['event_label'])
            stock = stock.append(slc)
            stock = stock.sort_index()

        self.df_log = stock
        print('MailTask')
        if stock[stock['Content'].str.contains("exception")].shape[0] > 3:
            mt = MailTask()
        ...
        self.df_log.to_csv(os.path.join(
            self.savePath, logname + '_structured.csv'), index=False)
        # print(str(count))
